...
public class PasswordStats implements IPasswordStats {
    ...
    public List<String> getAllWithNumbers(Supplier<Stream<String>> allPasswords) {
        //TODO
        return allPasswords.get().filter(hasNumber).collect(Collectors.toList());
        //System.out.println(allPasswords.get());
    ...
    public List<String> getAllWithUppercaseAndLowercase(Supplier<Stream<String>> allPasswords) {
        //TODO
        return allPasswords.get().filter(hasLowercase.and(hasUppercase)).collect(Collectors.toList());
    }
    ...
    public List<String> getAllWithSpecialChars(Supplier<Stream<String>> allPasswords) {
        //TODO
        return allPasswords.get().filter(hasSpecial).collect(Collectors.toList());
    }
    ...
    public List<String> getAllStrong(Supplier<Stream<String>> allPasswords) {
        //TODO
        return allPasswords.get().filter(isStrongPassword).collect(Collectors.toList());
    }
    ...
    public Map<Integer, List<String>> getAllBySpecialCharPosition(Supplier<Stream<String>> allPasswords) {
        ...
        return allPasswords.get()
                .filter(hasSpecial)
                .map(p ->getIndexOfSpecialChar(p)
                        .stream().map(a -> new AbstractMap.SimpleEntry<>(a,p))
    ...
    public List<String> getAllWithOnlyOneLastSpecialChar(Supplier<Stream<String>> allPasswords) {
        return allPasswords.get()
                .filter(hasSpecial)
                .filter(p -> getIndexOfSpecialChar(p)
                        .size()==1 && p.length()==getIndexOfSpecialChar(p)
                        .get(0)+1)
                .collect(Collectors.toList());
    }
}
